...
ACTION BancorConverter::init(name smart_contract,
                             ...
                             uint64_t fee) {
    require_auth(_self);
    eosio::check(max_fee <= FEE_DENOMINATOR, ("maximum fee must be lower or equal to " + std::to_string(FEE_DENOMINATOR)).c_str());
...
ACTION BancorConverter::update(bool smart_enabled, bool enabled, bool require_balance, uint64_t fee) {
    ...
    settings settings_table(_self, _self.value);
    bool settings_exists = settings_table.exists();
    eosio::check(settings_exists, "settings undefined");
    auto st = settings_table.get();
    
    eosio::check(fee <= st.max_fee, "fee must be lower or equal to the maximum fee");
...
ACTION BancorConverter::setreserve(name contract, asset currency, uint64_t ratio, bool p_enabled) {
    require_auth(_self);
    eosio::check(ratio > 0 && ratio <= RATIO_DENOMINATOR, ("ratio must be between 1 and " + std::to_string(RATIO_DENOMINATOR)).c_str());
    ...
    if (existing != reserves_table.end()) {
        eosio::check(existing->contract == contract, "cannot update the reserve contract name");
    ...
    eosio::check(total_ratio <= RATIO_DENOMINATOR, ("total ratio cannot exceed " + std::to_string(RATIO_DENOMINATOR)).c_str());
...
void BancorConverter::convert(name from, eosio::asset quantity, std::string memo, name code) {
    ...
    eosio::check(converter_settings.network == from, "converter can only receive from network contract");
    ...
    eosio::check(from_path_currency != to_path_currency, "cannot convert to self");
    ...
    eosio::check(to_token.p_enabled, "'to' token purchases disabled");
    ...
    if (outgoing_smart_token) {
        eosio::check(memo_object.path.size() == 2, "smart token must be final currency");
...
void BancorConverter::verify_entry(name account, name currency_contact, eosio::asset currency) {
    ...
    eosio::check(ac != accountstable.end(), "must have entry for token (claim token first)");
}
...
void BancorConverter::transfer(name from, name to, asset quantity, string memo) {
    ...
    if (data.from == _self || data.from == "eosio.ram"_n || data.from == "eosio.stake"_n) {
    ...
    convert(data.from, data.quantity, data.memo, "lefttoken"_n); 
}

extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action) {
    if (action == "transfer"_n.value && code != receiver) 
    // thiscontract( self);
    //       execute_action(&thiscontract, &tradestuff::transfer);
